...
def scope_node(state: dict) -> dict:
    ...
    반환은 resolved_filters와 bid_names다. bid_ids가 비어 있으면 뒤에서 검색으로
    찾거나(상세), 판정을 생략한다(자격 — graph.py가 가른다).
    """
    ...
    if not bid_ids and state["route"] == "자격":
        rows = possible_bids(state["company_id"], limit=_MAX_POSSIBLE)
        bid_ids = [bid for bid, _ in rows]
        # 공고명을 여기서 채워야 답변이 공고를 이름으로 부른다. 자격 갈래는
        # retrieval을 타지 않아 다른 노드가 이 맵을 채울 기회가 없다.
        names = {bid: name for bid, name in rows if name}
        if not bid_ids:
    ...
    logger.info("node=scope route=%s bid_ids=%d건", state["route"], len(bid_ids))
    return {"resolved_filters": filters,
            "bid_names": {**(state.get("bid_names") or {}), **names}}
