...
# 상한의 영향을 받지 않는다.
# ※ ADR 0007 이후에는 scope 노드가 후보를 5건으로 선캡하므로 현재 이 상한에
#   도달하는 경로가 없다 — 전건 경로(백엔드 직접 호출 등)의 안전망으로 유지한다.
_MAX_ELIGIBILITY_ROWS = 20

# 잘라야 한다면 무엇을 남길 것인가. 회원에게 쓸모 있는 순서다:
# 지금 되는 것 → 채우면 되는 것 → 우리가 모르는 것 → 가망 없는 것.
# '불가'가 전체의 2/3 이상(2026-07-30 v2.3 기준 899/1,305)이라, 정렬 없이
# 앞에서 20건을 자르면 화면이 전부 불가로 덮인다.
# ※ 같은 우선순위를 백엔드 추천 정렬에도 제안해 둠(backend_handover_2026-07-30).
#   '확인필요' 안에는 근거 없는 캡 산출(need_review==0, v2.2~2.3)이 섞여 있는데
#   여기 대화 경로는 굳이 거르지 않는다 — 특정 공고를 물으면 답해야 하므로.
