...
# ── 같은 이름의 다른 공고 (2026-08-05 실사용에서 발견) ─────────────
# 나라장터는 분할발주·재공고로 이름이 같은 공고가 흔하다. 예전 렌더러는 화면에
# 보이는 이름으로 묶어서 서로 다른 두 공고가 한 덩어리가 됐고, 사용자에게는
# "같은 내용이 두 번 적힌" 답변으로 보였다.

def _item(bid_id, label, text):
    return {"bid_id": bid_id, "label": label, "text": text}
...
def test_이름이_같아도_다른_공고면_머리줄을_따로_쓴다():
    ans = _render(
        [_item("R001_001", "마감", "2026-09-02 10:00"),
         _item("R001_002", "마감", "2026-09-02 10:00")],
        {"R001_001": "디벗 양품화 사업", "R001_002": "디벗 양품화 사업"})
    assert ans.count("- 디벗 양품화 사업") == 2, ans


def test_이름이_겹치면_공고번호로_구별해_준다():
    ans = _render(
        [_item("R001_001", "마감", "2026-09-02 10:00"),
         _item("R001_002", "마감", "2026-09-03 10:00")],
        {"R001_001": "디벗 양품화 사업", "R001_002": "디벗 양품화 사업"})
    assert "R001_001" in ans and "R001_002" in ans


def test_이름이_안_겹치면_공고번호를_붙이지_않는다():
    """평소에는 공고번호가 사용자에게 읽을 이유가 없는 식별자다."""
    ans = _render(
        [_item("R001_001", "마감", "2026-09-02 10:00"),
         _item("R002_001", "마감", "2026-09-03 10:00")],
        {"R001_001": "가 사업", "R002_001": "나 사업"})
    assert "공고번호" not in ans


def test_같은_공고의_여러_항목은_머리줄을_한_번만_쓴다():
    ans = _render(
        [_item("R001_001", "마감", "2026-09-02 10:00"),
         _item("R001_001", "추정가격", "10,806,403,636원")],
        {"R001_001": "디벗 양품화 사업"})
    assert ans.count("- 디벗 양품화 사업") == 1
    assert "마감" in ans and "추정가격" in ans


def test_같은_공고에_같은_항목이_두_번_오면_버린다():
    """모델이 항목을 되풀이해도 사용자 눈에는 렌더러 버그로 보인다."""
    ans = _render(
        [_item("R001_001", "마감", "2026-09-02 10:00"),
         _item("R001_001", "마감", "2026-09-02 10:00")],
        {"R001_001": "디벗 양품화 사업"})
    assert ans.count("2026-09-02 10:00") == 1, ans
